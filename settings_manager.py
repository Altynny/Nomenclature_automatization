...
class settings_manager(object) :
    ...
    def convert(self):
        ...
        print(self.__settings.TIN)

        field = "account"
        value = self.__data[field]
        setattr(self.__settings, field, value)
        
        print(self.__settings.account)

        field = "cor_account"
        value = self.__data[field]
        setattr(self.__settings, field, value)
        
        print(self.__settings.cor_account)

        field = "BIC"
        value = self.__data[field]
        setattr(self.__settings, field, value)
        
        print(self.__settings.BIC)

        field = "property_type"
        value = self.__data[field]
        setattr(self.__settings, field, value)
        
        print(self.__settings.property_type)
    ...
    def __open(self):
        ...
        """
        file_path = os.path.split(self.__file_name) #формируем из введённого имени список каталог/файл
        if not os.path.isdir(file_path[0]): file_path = os.path.split(__file__) #если не указан каталог, ищем в папке с проектом
